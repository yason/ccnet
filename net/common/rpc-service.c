                         ...
                         int is_staff, int is_active, GError **error)
{
    ...
    ret = ccnet_user_manager_add_emailuser (mgr, email, passwd,
                                            is_staff, is_active);
...
ccnet_rpc_remove_emailuser (const char *email, GError **error)
{
    ...
    ret = ccnet_user_manager_remove_emailuser (mgr, email);
...
ccnet_rpc_validate_emailuser (const char *email, const char *passwd, GError **error)
{
    ...
    ret = ccnet_user_manager_validate_emailuser (mgr, email, passwd);
...
{
    CcnetUserManager *user_mgr = session->user_mgr;
...
ccnet_rpc_add_binding (const char *email, const char *peer_id, GError **error)
{
    ...
    ret = ccnet_user_manager_add_binding (mgr, email, peer_id);
    if (ret == -2) {
...
ccnet_rpc_get_binding_email (const char *peer_id, GError **error)
{
    ...
    return ccnet_user_manager_get_binding_email (mgr, peer_id);
}
...
ccnet_rpc_get_binding_peerids (const char *email, GError **error)
{
    ...
    peerid_list = ccnet_user_manager_get_binding_peerids (mgr, email);
    if (peerid_list == NULL) {
                        ...
                        GError **error)
{
    ...
    ret = ccnet_group_manager_create_group (mgr, group_name, user_name, error);
...
ccnet_rpc_remove_group (int group_id, const char *user_name, GError **error)
{
    ...
    ret = ccnet_group_manager_remove_group (mgr, group_id, user_name, error);
                            ...
                            const char *member_name, GError **error)
{
    ...
    ret = ccnet_group_manager_add_member (mgr, group_id, user_name, member_name,
                                          error);
                               ...
                               const char *member_name, GError **error)
{
    ...
    ret = ccnet_group_manager_remove_member (mgr, group_id, user_name, member_name,
                                             error);
...
ccnet_rpc_quit_group (int group_id, const char *user_name, GError **error)
{
    ...
    ret = ccnet_group_manager_quit_group (mgr, group_id, user_name, error);
...
ccnet_rpc_get_groups (const char *username, GError **error)
{
    ...
    group_ids = ccnet_group_manager_get_groupids_by_user (mgr, username, error);
    if (group_ids == NULL) {
        return NULL;
    }
...
ccnet_rpc_get_group (int group_id, GError **error)
{
    ...
    group = ccnet_group_manager_get_group (mgr, group_id, error);
    if (!group) {
        return NULL;
    }
...
ccnet_rpc_get_group_members (int group_id, GError **error)
{
    ...
    ret = ccnet_group_manager_get_group_members (mgr, group_id, error);
    if (ret == NULL)
        return NULL;
                             ...
                             GError **error)
{
    ...
    return ccnet_group_manager_check_group_staff (session->group_mgr,
                                                  group_id, user_name);
}
